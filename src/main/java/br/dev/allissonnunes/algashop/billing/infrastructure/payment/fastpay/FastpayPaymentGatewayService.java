...
class FastpayPaymentGatewayService implements PaymentGatewayService {
    ...
    private final ResilientFastpayPaymentClient paymentClient;
    ...
    public Payment capture(final PaymentRequest request) {
        ...
        final FastpayPaymentModel paymentResponse = paymentClient.capture(input);
        return convertToPayment(paymentResponse);
    }

    @Override
    public Payment findByCode(final String gatewayCode) {
        final FastpayPaymentModel paymentResponse = paymentClient.findById(gatewayCode);
        return convertToPayment(paymentResponse);
    }
